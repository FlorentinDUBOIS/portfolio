...
<html lang="<?= ifindexsetor( $args, 'lang', DEFAULT_LANGUAGE ) ?>" >
    <head>
        ...
        <meta name="identifier-url"   lang="<?= ifindexsetor( $args, 'lang', DEFAULT_LANGUAGE ) ?>" content="http://<?= $_SERVER['SERVER_NAME'] ?>" />
        ...
        <?php
            ...
            foreach( $links as $path => $link ) {
                foreach( $link as $file ) {
                    echo '<link rel="stylesheet" type="'.Document::CSS.'" href="'.Document::file( $path.'/'.$file ).'" ></link>'.PHP_EOL;
                }
            }
